...
hf_API_key= "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"
...
EMOJI = {
    "person":"🧍","car":"🚗","truck":"🚚","bus":"🚌","bicycle":"🚲","motorcycle":"🏍️","dog":"🐶","cat":"🐱",
...
def infer(path, img_bytes, tries=8):
    ...
    for _ in range(tries):
        if mime and mime.startswith("image/"):
            ...
            r = requests.post(API,
                headers={"Authorization": f"Bearer {hf_API_key}"},
                files={"inputs": (os.path.basename(path),
                                  img_bytes, "application/octet-stream")},
                timeout=60)
...
def draw(img, dets, thr=0.5):
    ...
    for det in dets[:50]:
        ...
        x1,y1,x2,y2 = (int(b.get(k,0)) for k in 
    ("xmin","ymin","xmax","ymax"))
